...
def main():
    ...
    # ### Блок работы с архивом (без изменений) ###
    ...
    # --- АНАЛИЗ ДАННЫХ (### ИЗМЕНЕНО НА АСИНХРОННЫЙ ЗАПУСК) ---
    ...
    if session_key not in st.session_state:
        if selected_source == "Новый анализ" and client:
            with st.spinner('Выполняется параллельный анализ рефлексий... Это будет быстрее!'):
                # 1. Создаем список асинхронных задач (корутин)
                tasks = [analyze_reflection_with_deepseek(client, text) for text in df['text']]

                # 2. Создаем асинхронную функцию для их параллельного выполнения
                async def gather_tasks():
                    # asyncio.gather запускает все задачи одновременно и ждет их завершения
                    return await asyncio.gather(*tasks)

                # 3. Запускаем асинхронный код из синхронного окружения Streamlit
                results = asyncio.run(gather_tasks())
                
                # Дальнейшая обработка результатов как и раньше
                results_df = pd.DataFrame(results)
                df = pd.concat([df.reset_index(drop=True), results_df.reset_index(drop=True)], axis=1)
        
        # Конвертируем оценки (без изменений)
    ...
    # ### Кнопка сохранения в архив (без изменений) ###
    ...
    # --- ФИЛЬТРЫ И ВИЗУАЛИЗАЦИЯ (далее код без изменений) ---
    # ... (весь ваш код для построения графиков, таблиц и т.д. остается здесь без изменений) ...
    # Я скопирую его для полноты, чтобы скрипт был рабочим
    filtered_df = df.copy()
